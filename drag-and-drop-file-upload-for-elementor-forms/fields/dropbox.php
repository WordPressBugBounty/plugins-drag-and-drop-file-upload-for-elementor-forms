...
class Yeeaddons_EL_Dropbox_API_Setttings
{
    ...
    public function register_admin_fields(Settings $settings)
    {
        ...
        $settings->add_section(Settings::TAB_INTEGRATIONS, 'yeeaddons_dropbox_api', [
            'callback' => function () {
                echo '<hr><h2>' . esc_html__('Dropbox API Drag and Drop File Upload', 'drag-and-drop-file-upload-for-elementor-forms') . '</h2>';
            },
            'fields' => [
                self::OPTION_NAME_API_KEY => [
                    'label' => esc_html__('Client id', 'drag-and-drop-file-upload-for-elementor-forms'),
                    'field_args' => [
                        'type' => 'text',
                        'desc' => sprintf(
                            /* translators: 1: Link opening tag, 2: Link closing tag. */
                            esc_html__('Document %1$ssee more%2$s.', 'drag-and-drop-file-upload-for-elementor-forms'),
                ...
                "yeeaddons_dropbox_client_secret" => [
                    'label' => esc_html__('Client secret', 'drag-and-drop-file-upload-for-elementor-forms'),
                    'field_args' => [
                        'type' => 'text',
                    ],
                ],
                "yeeaddons_dropbox_access_code" => [
                    'label' => esc_html__('Access Code', 'drag-and-drop-file-upload-for-elementor-forms'),
                    'field_args' => [
                        'type' => 'text',
                        'desc' => sprintf('<a id="yeeaddons_dropbox_api_getcode" href="https://www.dropbox.com/oauth2/authorize?client_id=%s&response_type=code&token_access_type=offline" target="_blank">%s</a>', $clientId, esc_html__('Get Access Code', 'drag-and-drop-file-upload-for-elementor-forms')),
                    ],
                ],
                'validate_api_data' => [
                    'field_args' => [
                        'type' => 'raw_html',
                        'html' => sprintf('<button data-action="%s" data-nonce="%s" class="button elementor-button-spinner" id="elementor_pro_yeeaddons_dropbox_api_key_button">%s</button>', self::OPTION_NAME_API_KEY . '_validate', wp_create_nonce(self::OPTION_NAME_API_KEY), esc_html__('Validate API Key', 'drag-and-drop-file-upload-for-elementor-forms')),
                    ],
                ],
            ],
        ]);
